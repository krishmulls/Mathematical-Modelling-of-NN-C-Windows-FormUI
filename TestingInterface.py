...
model = model_from_json(open("D:\Workspace\VS\ResearchMethods\Traffic1.json", "r").read())
#load weights
model.load_weights('D:\Workspace\VS\ResearchMethods\Traffic1.h5')
...
def show_classify_btn(file_path):
  ...
  usertrain=Button(top,text="Start Retrain",command=retrain_func,padx=10,pady=10)
  usertrain.configure(background='#364156', fg='white',font=('arial',10,'bold'))
  usertrain.pack(side=BOTTOM,pady=50)
...
def retrain_func():
  try:
    ...
    os.system("python.exe D:\Workspace\VS\ResearchMethods\System_Learning.py")
  except:
    pass
...
if __name__=="__main__":
  ...
  upload=Button(top,text="Upload Image to test neural network",command=upload_image,padx=10,pady=5)
